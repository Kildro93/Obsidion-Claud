# Gedächtnis & Kontext-Index

Einstiegspunkt für jeden Chat. Zuerst hier, dann [[Regeln]] und [[Profil]] lesen.

## Über mich & Kernregeln
- Wer ich bin: [[Profil]]
- Kommunikationsstil & No-Gos: [[Regeln]]
- Vault-Aufbau & Loop-System: [[Vault-Struktur]]
- Schnelleinstieg für neue Chats/Bots: [[QUICK-START]]

## Betrieb & Zugänge
- Zugangs-Übersicht (nur Templates): [[PROJEKT-CREDENTIALS]]
- GitHub-Token: [[SETUP-GITHUB-TOKEN]] · Firebase: [[SETUP-FIREBASE]] · Lokale Konfig: [[SETUP-ENV-LOCAL]]
- Auto-Sync: [[AUTO-SYNC]] · Backups: [[BACKUP-STRATEGY]] · Audit: [[SECURITY-AUDIT]]
- Fehlersuche: [[DEBUGGING]]

## Projekte

### Nestbau (Hauptprojekt) – Household-Management-App
- Projekt-Einstieg: `PROJEKTE/Nestbau/README.md` ← Start hier
- Struktur & Rollen: [[PROJEKT-LOOP]] · Aktueller Status: [[PROJEKT-UPDATE]]
- Erkenntnisse: [[PROJEKT-LEARNINGS]] · Zugänge: [[PROJEKT-ACCESS]]
- Anforderungen: [[Features]] · [[User-Stories]] · [[Tech-Stack]]
- Design: [[Design-System]] · [[User-Flows]]
- Code-Landkarte: [[CODE-Landkarte]] · Verknüpfungen: [[VERKNUEPFUNGEN]]
- Feature-Übersicht: [[NESTBAU_AKTUELL]] · Changelog: [[Learnings]] · Artifacts: [[ARTIFACT_LINKS]]
- Knowledge Base: [[NESTBAU-KNOWLEDGE-INDEX]] (Technik: [[nestbau-tech]], Testing: [[nestbau-testing]], Produkt: [[haushalts-app]])
- Sessions: [[INDEX|Chat-Exports/INDEX]]

### GitHub-Automation – Tool
- Übersicht: [[_INDEX]] (PROJEKTE/GitHub-Automation/)
- HTML-App zum Anlegen von GitHub-Repos aus Notizen. Kein Bot-Loop.

## Weitere Bereiche
- Unterrichtsvorbereitung: LERNEN/Primarlehrer/
- Technik / Anime: LERNEN/
- Koch-Wissen: KOCH-WISSEN/
- Alltags-Routinen: ROUTINEN/

---
*Zuletzt aktualisiert: 2026-09-06 – Vault-Repo initialisiert, Setup/Auto-Sync/Backup/Audit ergänzt*
