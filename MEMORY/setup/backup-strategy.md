...
Auto-Sync alle 30 Minuten, siehe [[MEMORY/setup/auto-sync]]. Deckt alles ab, was im Vault-Repo liegt. Deckt NICHT ab: die per `.gitignore` ausgeschlossenen Dateien — Keystore, `nb-config.local.js`, Backups, `Nestbau/`, `nestbau-firebase/` (die haben eigene Repos).
...
- Start ueber `wscript.exe` + `weekly-backup-silent.vbs` (kein sichtbares Fenster, siehe [[MEMORY/setup/auto-sync]])
