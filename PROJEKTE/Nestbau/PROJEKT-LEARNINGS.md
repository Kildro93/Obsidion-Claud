...
**2026-09-13 – Google-Kalender Token-Refresh (Auth-Bot)**
- GIS bietet zwei Modi: `initTokenClient` (Implicit, kein Refresh-Token, rein clientseitig) und `initCodeClient` (Code, braucht Backend-Tausch mit Client-Secret). Wer dauerhaften Server-Zugriff auf Google-APIs braucht, kommt um Letzteres + eigenes Backend nicht herum. Der Brief ging von "Function deployen + Frontend anbinden" aus — tatsächlich war ein voller Umbau des Connect-Flows nötig, weil das Frontend GIS Implicit nutzte und die Functions-SDK gar nicht geladen war.
- Bot-Briefs müssen den IST-Zustand des Codes beschreiben, nicht den Soll-Zustand aus der Architektur-Doku. Der Brief nahm an, `tokens.js` sei "fast fertig" — tatsächlich war die Function für einen serverseitigen Sync-Job gebaut (Access-Token sollte laut Kommentar "nie an Client"), der nicht existiert. Der Bot hat das korrekt erkannt, eskaliert, und nach Freigabe den vollen Umbau sauber durchgezogen. Regel: vor jedem Bot-Brief den relevanten Code selbst lesen oder den Bot in Batch 1 explizit zur Bestandsaufnahme mit Eskalation briefen.
- Zwei-Repo-Situation (Nestbau + nestbau-firebase) hat die Aufgabe spürbar verlangsamt — kein Callable war verdrahtet, Functions-SDK fehlte komplett. Der Repo-Merge gewinnt dadurch an Dringlichkeit.
- Bestehende Google-Verbindungen brauchen nach dem Deploy eine einmalige Neuverbindung — der alte Implicit-Flow hat nie einen serverseitigen Refresh-Token erzeugt. Kein automatischer Migrationspfad möglich.
